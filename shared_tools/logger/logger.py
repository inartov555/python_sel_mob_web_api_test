...
class Logger:
    ...
    def __init__(self, logger_name: str):
        ...
        Logging path is LOG_PATH if defined, else it's the project root directory.
    ...
    def __update_handler(self, logr, handlr):
        """
        Update the logger's handler with a new log level. The method will get all handlers of the logger and change the level
    ...
    def setup_cli_handler(self, level: str):
        ...
        If the CLI handler was set up for all loggers, then all new ones will be created with the same config
    ...
    def setup_filehandler(self, file_name: str, level: str = "DEBUG"):
        """
        Method to setup file handler for a particular logger or all available
